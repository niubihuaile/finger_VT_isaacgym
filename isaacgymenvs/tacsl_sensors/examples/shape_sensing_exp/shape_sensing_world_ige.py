...
class TactileEnv:
    ...
    def create_assets(self, table_dims, indenter_name):
        ...
        elif self.sensor_type == 'fingertip':
            self.gelsight_asset_file = 'urdf/gelsight_description/robots/fingertip.urdf'
        else:
    ...
    def create_envs_and_actors(self, table_dims):
        ...
        if self.sensor_type == 'fingertip':
            gelsight_pose.r = gymapi.Quat(*R.from_euler('xyz', [0, 0, 0]).as_quat())
        ...
        print(f'Indenter pose: {indenter_pose.p}')
...
def parse_args():
    ...
    custom_parameters = [
        ...
        {'name': '--action_duration', 'type': float, 'default': 10.0, 'help': 'Number of seconds for each indenting action'},
    ]
