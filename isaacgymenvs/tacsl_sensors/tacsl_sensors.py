...
def get_camera_config(sensor_type, tactile_camera_name, actor_name, attach_link_name):
    ...
    参数:
        sensor_type (str): 传感器的类型（例如，'gelsight_r15'，'gs_mini'）。
        tactile_camera_name (str): 触觉相机的名称。
        actor_name (str): 触觉传感器附着的角色名称。
        attach_link_name (str): 相机附着的链接名称。
    ...
    if sensor_type == 'gelsight_r15':
        ...
        config_filepath = os.path.join(parent_dir, 'configs', 'finger_tip.yaml')
    else:
    ...
    if sensor_type == 'fingertip':
        # s手动更换相机位置 （0.00883, 0， 0.01251）
        # tip_to_cam_pos = [0.0, 0.0, -tactile_camera_config['camera_dist']]
        tip_to_cam_pos = [0.00883, 0.0, -0.01251]
    else:
        tip_to_cam_pos = [0.0, 0.0, -tactile_camera_config['camera_dist']]
...
class TactileBase:
    ...
    Provides methods for configuring compliant dynamics. 配置柔性动力学基类
    """
    ...
    def set_compliant_dynamics_for_tactile_sensors(self, tactile_sensor_configs):
        ...
        Args:
            tactile_sensor_configs (list): List of tactile sensor configurations. 为多个触觉传感器配置柔性动力学
        """
